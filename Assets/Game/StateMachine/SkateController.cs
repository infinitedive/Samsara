...
public class SkateCharacter : PlayerCharacter {
    ...
    private void Update () {
        ...
        speedBall.transform.forward = moveData.velocity.normalized;
 
        speedBallSquash = speedBall.transform.InverseTransformVector(squash);

        // float squashX = Vector3.Dot(speedBallSquash, speedBall.transform.right) / 10f;
        // float squashY = Vector3.Dot(speedBallSquash, speedBall.transform.up) / 10f;
        // float squashZ = Vector3.Dot(speedBallSquash, speedBall.transform.forward) / 10f;

        squash = Vector3.Lerp(squash, Vector3.zero, Time.deltaTime / 4f);

        float squashY = Vector3.Dot(squash, Vector3.up) / 5f;
    ...
    private void CheckGrapplePress() {
        ...
        if (playerData.wishGrapplePress && !playerData.grappling && energySlider.value > .1f) {
    ...
    private void TransformRotation() {
        ...
        focusOnThis.position = Vector3.Lerp(focusOnThis.position, playerData.targetMean, Time.deltaTime * 10f);
    ...
    private void LinearChange(float start, float end, float delta) {
    ...
    private void CameraSettings() { // TODO:
        ...
        if (playerData.mainTarget.position != Vector3.zero) {
            ...
            float cameraSideDamp = 2f;
            float toTargetCS = 1f - followCam.CameraSide;
            followCam.CameraSide += toTargetCS * Time.deltaTime * cameraSideDamp;
    ...
    void DrawRope() {
        ...
        if (!playerData.grappling) return;
        ...
        _lr.SetPosition(1, playerData.grapplePoint);
