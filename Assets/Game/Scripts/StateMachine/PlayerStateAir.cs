...

{
    ...
    public PlayerStateAir(Controllers.CharacterController currentContext, PlayerStateFactory playerStateFactory) : base (currentContext, playerStateFactory) {
        _isRootState = true;
        name = "air";
    }
    ...
    {
        ctx.animationController.animator.SetBool("falling", ctx.characterData.moveData.velocity.y < 0f);
        ...
        if (ctx.characterData.playerData.detectWall) {
            ...
            ctx.characterData.moveData.velocity.y = Mathf.Lerp(ctx.characterData.moveData.velocity.y, 0f, Time.deltaTime);

            ctx.animationController.animator.Play("h_neutral");

            if (ctx.characterData.playerData.wishJumpUp && ctx.timerController.wallTouchTimer <= 0f && ctx.characterData.playerData.detectWall) {
                WallJump((ctx.characterData.playerData.wallNormal + Vector3.up).normalized * 1f);
                ctx.timerController.wallTouchTimer = .5f;
            }
        }
        else {
            ctx.characterData.playerData.falling = true;
        }
    ...
    protected void WallJump(Vector3 direction) {
